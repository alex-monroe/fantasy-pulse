...
export default function SleeperPage() {
  ...
  useEffect(() => {
    const checkIntegration = async () => {
      const { integration, error } = await getIntegrationForProvider('sleeper');
      if (error) {
        setError(error);
      } else {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setError(null);
    const { error: userError } = await connectProvider('sleeper', username);
    if (userError) {
      setError(userError);
      return;
    }
    const { integration, error: integrationError } = await getIntegrationForProvider('sleeper');
    if (integrationError) {
  ...
  const handleRemove = async () => {
    ...
    const { error } = await removeProvider('sleeper', integration.id);
  ...
  const handleFetchMatchups = async (leagueId: string, week: string) => {
    ...
    setError(null);
    const { matchups, error } = await getLeagueMatchups('sleeper', leagueId, week);
    if (error) {
      setError(error);
    } else {
  ...
  useEffect(() => {
    if (integration) {
      const fetchLeagues = async () => {
        const { leagues, error } = await getLeaguesForProvider('sleeper', integration.id);
        if (error) {
          setError(error);
          return;
        }
