...
export async function getYahooAccessToken(integrationId: number): Promise<{ access_token?: string; error?: string }> {
  ...
  if (integration.expires_at && new Date(integration.expires_at).getTime() < Date.now() + 60000) {
    ...
    try {
      ...
      if (error || !data) {
        logDuration('yahoo:refreshAccessToken', refreshStart, {
          integrationId,
          success: false,
          errorMessage: error || 'No response body returned',
        });
...
export async function getYahooUserTeams(integrationId: number) {
  ...
  try {
    ...
    if (error || !data) {
      logDuration('yahoo:getUserTeams', requestStart, {
        integrationId,
        success: false,
        errorMessage: error || 'No response body returned',
      });
    ...
    logDuration('yahoo:getUserTeams', requestStart, {
      integrationId,
      success: true,
      teamCount: teamsToInsert.length,
    });
...
export async function getYahooLeagues(integrationId: number) {
  ...
  try {
    ...
    if (error || !data) {
      logDuration('yahoo:getLeagues', requestStart, {
        integrationId,
        success: false,
        errorMessage: error || 'No response body returned',
      });
    ...
    logDuration('yahoo:getLeagues', requestStart, {
      integrationId,
      success: true,
      leagueCount: leaguesToInsert.length,
    });
...
export async function getYahooRoster(
  ...
  try {
    ...
    if (error || !data) {
      logDuration('yahoo:getRoster', requestStart, {
        integrationId,
        leagueId,
        teamId,
        success: false,
    ...
    const players = Object.values(rosterData)
      ...
      .map((p: any) => {
        ...
        return {
          ...
          eligible_positions: playerDetails.eligible_positions?.map(
...
export async function getYahooMatchups(
  ...
  try {
    ...
    if (error || !data) {
      logDuration('yahoo:getMatchups', requestStart, {
        integrationId,
        teamKey,
        success: false,
        week: resolvedWeek,
    ...
    logDuration('yahoo:getMatchups', requestStart, {
      ...
      week: resolvedWeek,
      hasMatchup: true,
      opponentTeamKey: parsedOpponentTeam.team_key,
    });
...
export async function getYahooPlayerScores(
  ...
  try {
    ...
    if (error || !data) {
      logDuration('yahoo:getPlayerScores', requestStart, {
        integrationId,
        teamKey,
        success: false,
        week: resolvedWeek,
